...
function getBanner (filename) {
	return `/*! ${configs.name ? configs.name : pkg.name} - ${filename} v${pkg.version} | (c) ${new Date().getFullYear()} ${pkg.author.name} | ${pkg.license} License | ${pkg.repository.url} */`;
}
	...
	sass.render(getOptions(file, filename, minify), function (err, result) {
		...
		fs.mkdir(configs.pathOut, { recursive: true }, function (err) {
			...
			fs.writeFile(`${configs.pathOut}/${filename}`, result.css, function (err){
				...
				var insert = new Buffer.from(getBanner(file.replace('.scss', '')) + "\n");
